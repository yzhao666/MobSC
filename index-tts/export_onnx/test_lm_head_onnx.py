...
def load_pytorch_model(ckpt_path: str, config_path: str, device: torch.device):
    ...
    return model.final_norm, model.mel_head, model.model_dim, model.number_mel_codes


def create_lm_head_wrapper(final_norm, mel_head):
    ...
    return LMHeadWrapper(final_norm, mel_head)
...
def test_accuracy(
    ...
    for i in range(num_tests):
        ...
        hidden_states = torch.randn(1, seq_len, model_dim, device=device, dtype=torch.float32)
        
        # PyTorch 推理
        with torch.no_grad():
            pytorch_output = pytorch_model(hidden_states)
...
def test_output_distribution(pytorch_model, onnx_path: str, device: torch.device, model_dim: int, vocab_size: int):
    """测试输出分布的一致性"""
    print(f"\n>> 测试输出分布一致性...")
    ...
    # 生成测试输入
    hidden_states = torch.randn(1, 32, model_dim, device=device, dtype=torch.float32)
    ...
    print(f"    形状: {pytorch_np.shape}")
    print(f"    均值: {pytorch_np.mean():.6f}")
    print(f"    标准差: {pytorch_np.std():.6f}")
    print(f"    最小值: {pytorch_np.min():.6f}")
    print(f"    最大值: {pytorch_np.max():.6f}")
    
    print(f"  ONNX 输出统计:")
    print(f"    形状: {onnx_output.shape}")
    print(f"    均值: {onnx_output.mean():.6f}")
    print(f"    标准差: {onnx_output.std():.6f}")
    print(f"    最小值: {onnx_output.min():.6f}")
    print(f"    最大值: {onnx_output.max():.6f}")
    
    # 计算分布差异
    mean_diff = abs(pytorch_np.mean() - onnx_output.mean())
    std_diff = abs(pytorch_np.std() - onnx_output.std())
...
def main():
    parser = argparse.ArgumentParser(description="测试 lm_head.onnx 精度")
    ...
    parser.add_argument("--onnx", type=str, default="checkpoints_onnx/lm_head.onnx", help="ONNX 模型路径")
    ...
    final_norm, mel_head, model_dim, vocab_size = load_pytorch_model(args.ckpt, args.config, device)
    
    print(f">> 创建 PyTorch 包装器")
    pytorch_wrapper = create_lm_head_wrapper(final_norm, mel_head).to(device).eval()
    ...
    # 输出分布测试
    test_output_distribution(pytorch_wrapper, args.onnx, device, model_dim, vocab_size)
